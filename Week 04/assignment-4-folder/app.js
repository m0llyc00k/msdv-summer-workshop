...
function draw() {
  ...
  textSize(16);
  textFont('calibri');
  text("Studio Ghibli Ratings from Rotten Tomatoes", mouseX, mouseY);
  pop();
...
function createBar(entry,index){
    ...
    let leftMargin = 75;
    let barHeight = entry.rt_score * scale;
    let barWidth = 30;
    let barSpacing = 1;
    fill(235, 131, 154);
    ...
    push();
    noStroke();
    fill((entry.rt_score / 130) * 149, 51, 98);
    rect(x,y, barWidth, barHeight, entry.rt_score*0.3);
    pop();

    
    push();
    translate(x + barWidth/2, 460);
    rotate(Math.PI * 45 / 120);
    text(entry.title + '---' + entry.release_date, 0, 0);
    pop();

  
    
    push();
    translate(x + barWidth/2, 475 - barHeight)
    textAlign(CENTER);
    text(entry.rt_score, 0, 0);
    pop();
...
//   var barWidth =  (h / myData.length) * 0.8; // width of bar
//   var barMargin = (h / myData.length) * 0.2; // margin between two bars
...
//     rect(0, 0, myData[i], barWidth); // draw rect

//     fill('#FFF');
//     text(myData[i], 5, barWidth/2 + 5); // write data
